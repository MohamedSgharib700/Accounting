...
<div class="col-span-12 sm:col-span-12 xl:col-span-12 intro-y my-10" style="text-align:center">
    ...
    <h1 class="p-3 "> الماكينات الغير مفعله </h1>
</div>
...
<div class="col-span-12 sm:col-span-12 xl:col-span-12 intro-y my-10">
    <form class="" style="display:flex;flex-wrap: wrap;">
        ...
        <div>
            <button type="submit" class="button w-30 mr-1 mb-2 bg-theme-12 text-white m-2" id="excel" data-id=""
        ...
        <tbody>
            ...
            <tr class="pos-{{$row->id}} intro-x">
                ...
                <td class="text-center">
                    <div class="flex items-center justify-center "> {{$row->serial}} </div>
                </td>


                <td class="w-40">
                    <div class="flex items-center justify-center text-theme-9">
                        {{$row->machine_code}}</div>
                </td>
                <td class="text-center">
                    <div class="flex items-center justify-center ">
                ...
                <td class="text-center">
                    <div class="flex items-center justify-center "> {{$row->customer ? $row->customer->created_at: ''}}</div>
                </td>
                <td class="text-center">
                    <div class="flex items-center justify-center "> {{$row->installments->first->value ? $row->installments->first->value->contract->name : ''}}</div>
                </td>
                <td class="text-center">
                    <div class="flex items-center justify-center ">{{ ($row->transactions()->sum('value')) }}</div>
                </td>
                <td class="text-center">
                    <div class="flex items-center justify-center ">  
                     @php
...
$(document).ready(function() {
    ...
    $(document).on("click", "#excel", function(e) {
        ...
        $.ajax({
            type: "GET",
            Content: "text/csv",
            url: 'export-all-available-pos',
            success: function(data) {
                window.location.href = 'export-all-available-pos';
            }
        });
        e.preventDefault();
    });
    ...
    $(document).on("click", "#pdf", function(e) {
        ...
        $.ajax({
            type: "GET",
            Content: "text/pdf",
            url: 'export-all-available-pos-pdf',
            success: function(data) {
                window.location.href = 'export-all-available-pos-pdf';
            }
        });
        e.preventDefault();
    });
