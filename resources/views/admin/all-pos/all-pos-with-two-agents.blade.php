    ...
    <table class="table table-report text-center  large-table -mt-2">
        ...
        <tbody>
            ...
            <tr class="pos-{{$row->id}} intro-x">
                ...
                <td >
                    <div class="flex items-center justify-center">
                        <div class=" h-10 ">{{$row->pos_note->where('status','=','1')->first->value('old_agent_id')? $row->pos_note->first->value('old_agent_id')->old_agent->name: ''}}</div>
                    </div>
                </td>
                <td>
...
$(document).ready(function() {
    ...
    $(document).on("click", "#excel", function(e) {
        ...
        $.ajax({
            type: "GET",
            Content: "text/csv",
            url: 'export-all-two-agents-pos?from_date=' + from_date + '&to_date=' + to_date,
            success: function(data) {
                window.location.href = 'export-all-two-agents-pos?from_date=' + from_date +
                    '&to_date=' + to_date;
            }
        });
        e.preventDefault();
